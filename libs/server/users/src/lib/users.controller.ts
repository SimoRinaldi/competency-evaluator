...
export class UsersController {
  ...
  async updateMe(@CurrentUser() user: any, @Body() body: any) {
    ...
    const { role, oldPassword: old_password, newPassword: new_password, ...allowed_updates } = body;
    ...
    if (old_password && new_password) {
      await this.usersService.updatePassword(user.id, old_password, new_password);
    }

    if (Object.keys(allowed_updates).length > 0) {
      await this.usersService.update(user.id, allowed_updates);
    }
