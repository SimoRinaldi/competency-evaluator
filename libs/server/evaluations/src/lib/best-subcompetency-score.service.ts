...
@Injectable()
export class ServerBestSubCompetencyScoresService {
    constructor( 
        private readonly bestSubCompetencyScoresRepository: ServerBestSubCompetencyScoresRepository 
    ){}
    ...
    async findOne(id: number): Promise<BestSubCompetencyScoreEntity> {
        ...
        if (!bscs) 
            throw new NotFoundException(`Best SubCompetency Score con ID ${id} non trovato.`);

        return bscs;
    }

    async update(id: number, dto: UpdateBestSubCompetencyScoreDto) {
        const bscs = await this.bestSubCompetencyScoresRepository.findById(id);
        if (!bscs) {
            throw new NotFoundException(`Best SubCompetency Score ${id} non trovato.`);
        }
