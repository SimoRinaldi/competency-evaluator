...
@Injectable()
export class ServerBestCompetencyScoresRepository {
    constructor(
    ...
    async updateOne(bcs: BestCompetencyScoreEntity, dto: UpdateBestCompetencyScoreDto): Promise<BestCompetencyScoreEntity> {
        if (dto.best_score_absolute !== undefined) bcs.best_score_absolute = dto.best_score_absolute
        if (dto.best_score_percentage !== undefined) bcs.best_score_percentage = dto.best_score_percentage;
        if (dto.user_id !== undefined) bcs.user_id = dto.user_id;
        if (dto.competency_id !== undefined) bcs.competency_id = dto.competency_id;
